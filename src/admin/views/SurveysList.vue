<template>
  <div class="pollquest-content-inner">
    ...
    <div class="pollquest-page-header">
      <div>
        <h1 class="pollquest-page-title">Surveys</h1>
        <p class="pollquest-page-subtitle">Create, publish and manage the surveys running on your site.</p>
      </div>
      <div>
        <button type="button" class="pollquest-btn pollquest-btn-primary" @click="openTemplatePicker">
          <Plus />
          New survey
        </button>
      </div>
    ...
    <!-- Stats -->
    <div class="pollquest-stats-grid">
      <div class="pollquest-stat-card">
        <div class="pollquest-stat-label">Total</div>
        <div class="pollquest-stat-value">{{ counts.all || 0 }}</div>
      </div>
      <div class="pollquest-stat-card">
        <div class="pollquest-stat-label">Published</div>
        <div class="pollquest-stat-value">{{ counts.publish || 0 }}</div>
      </div>
      <div class="pollquest-stat-card">
        <div class="pollquest-stat-label">Drafts</div>
        <div class="pollquest-stat-value">{{ counts.draft || 0 }}</div>
      </div>
      <div class="pollquest-stat-card">
        <div class="pollquest-stat-label">Trashed</div>
        <div class="pollquest-stat-value">{{ counts.trash || 0 }}</div>
      </div>
    </div>
    ...
    <div class="pollquest-surveys-panel">
      <!-- Toolbar -->
      <div class="pollquest-panel-toolbar">
        <div class="pollquest-filter-tabs">
          <button
            v-for="tab in tabs"
            :key="tab.value"
            class="pollquest-filter-tab"
        ...
        <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
          ...
          <template v-if="selected.length > 0">
            ...
            <button class="pollquest-btn pollquest-btn-secondary pollquest-btn-sm" @click="bulkAction('publish')">Publish</button>
            <button class="pollquest-btn pollquest-btn-secondary pollquest-btn-sm" @click="bulkAction('draft')">Draft</button>
            <button class="pollquest-btn pollquest-btn-danger pollquest-btn-sm" @click="bulkAction('trash')">Trash</button>
          </template>
      ...
      <!-- Column Headers -->
      <div class="pollquest-table-header">
        <div>Name</div>
        <div>Status</div>
      ...
      <ul v-if="filteredSurveys.length > 0">
        ...
        >
          ...
          <div style="min-width:0;">
            <div class="pollquest-survey-name truncate">{{ survey.title }}</div>
            <div class="pollquest-survey-type">{{ survey.type }}</div>
          </div>

          <!-- Status -->
          <div class="pollquest-status-dot-wrap">
            <span class="pollquest-ping-wrap">
              <span class="pollquest-ping-ring" :class="survey.status !== 'publish' ? 'draft' : ''" />
              <span class="pollquest-ping-dot" :class="survey.status !== 'publish' ? 'draft' : ''" />
            </span>
            <span class="pollquest-status-text">{{ statusLabel(survey.status) }}</span>
          </div>

          <!-- Responses -->
          <div class="pollquest-stat-cell right">{{ survey.responses || 0 }}</div>

          <!-- Views -->
          <div class="pollquest-stat-cell right">{{ survey.impressions || 0 }}</div>

          <!-- Completion -->
          <div class="pollquest-stat-cell right muted">
            {{ survey.impressions > 0 ? Math.round(((survey.responses || 0) / survey.impressions) * 100) : 0 }}%
          </div>

          <!-- Created -->
          <div class="pollquest-stat-cell muted">{{ formatDate(survey.created_at) }}</div>

          <!-- Actions -->
          <div class="pollquest-survey-actions">
            <template v-if="survey.status !== 'trash'">
              <router-link :to="`/surveys/${survey.id}/edit`">
                <button class="pollquest-icon-btn" title="Edit">
                  <Pencil />
                </button>
              </router-link>
            ...
            <template v-else>
              <button class="pollquest-btn pollquest-btn-secondary pollquest-btn-sm" @click="restoreSurvey(survey.id)">Restore</button>
              <button class="pollquest-btn pollquest-btn-danger pollquest-btn-sm" @click="deleteSurvey(survey.id)">Delete</button>
            </template>
          </div>
        </li>
      </ul>
